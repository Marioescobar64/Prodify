...
const axiosBackendA = axios.create({
    baseURL: import.meta.env.VITE_BACKENDA_URL,
    timeout: 8000,
});

const axiosBackendB = axios.create({
    baseURL: import.meta.env.VITE_BACKENDB_URL,
    timeout: 8000,
});
...
const handleRefreshToken = async function (_error) {
  ...
  if (shouldRefresh) {
    ...
    if (_original._axiosClient === "backendA") retryClient = axiosBackendA;
    if (_original._axiosClient === "backendB") retryClient = axiosBackendB;
...
axiosBackendA.interceptors.response.use((res) => res, handleRefreshToken);
axiosBackendB.interceptors.response.use((res) => res, handleRefreshToken);
 
// ================= EXPORT AXIOS =================
export { axiosAuth, axiosBackendA, axiosBackendB, handleRefreshToken };
